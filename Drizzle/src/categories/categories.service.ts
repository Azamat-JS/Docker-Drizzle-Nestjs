...
export class CategoriesService {
    ...
    async createCategory(category: typeof schema.categories.$inferInsert, tx?: NodePgTransaction<any, any>){
      return  (await (tx || this.database).insert(schema.categories).values(category).returning({id: schema.categories.id}))[0]
    
    }

    async addToPost(postToCategory: typeof schema.postsToCategories.$inferInsert, tx?: NodePgTransaction<any, any>){
